# Author      : Abhishek Lolage
# Filename    : vEB.py
# Description : This file contains the implementation of the van Emde Boas tree which supports
#               find, insert, delete, successor, predecessor in O(lg lg u) time. ``u'' is the size of the 
#               universe. max and min operations are supported in O(1) time.
# Notes       : The implementation follows from CLRS algorithms in Chapter 20
...
class VEBtree:
	...
	def vebTreeDelete(self, x):
		...
		if self.min == self.max:
			...
			if self.clusters[self.high(x)].getMin() == None:
